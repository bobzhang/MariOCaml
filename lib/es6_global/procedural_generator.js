...
function mem_loc(checkloc, _loclist) {
  while(true) {
    ...
    if (loclist) {
      if (Mario_util.eq_checkloc(checkloc, loclist[0][1])) {
        return /* true */1;
      } else {
        _loclist = loclist[1];
        continue ;
...
function generate_coins(_block_coord) {
  while(true) {
    ...
    var place_coin = Js_math.random_int(0, 2);
    if (block_coord) {
      var t = block_coord[1];
      var h = block_coord[0];
      if (place_coin) {
...
function choose_block_pattern(blockw, blockh, cbx, cby, prob) {
  if (cbx > blockw || cby > blockh) {
    ...
    var block_typ = Js_math.random_int(0, 4);
    var stair_typ = Js_math.random_int(0, 2);
    var life_block_chance = Js_math.random_int(0, 5);
    ...
    if (prob > 5 || prob < 0) {
      ...
      switch (prob) {
        ...
        case 1 : 
            var num_clouds = Js_math.random_int(0, 5) + 5 | 0;
            if (cby < 5) {
...
function generate_enemies(blockw, blockh, _cbx, _cby, acc) {
  while(true) {
    ...
    if (cbx > blockw - 32) {
      ...
      var prob = Js_math.random_int(0, 30);
...
function generate_block_enemies(_block_coord) {
  while(true) {
    var block_coord = _block_coord;
    var place_enemy = Js_math.random_int(0, 20);
    var enemy_typ = Js_math.random_int(0, 3);
    if (block_coord) {
      var t = block_coord[1];
      var h = block_coord[0];
      if (place_enemy) {
...
function generate_block_locs(blockw, blockh, _cbx, _cby, _acc) {
  while(true) {
    ...
    if (blockw - cbx < 33) {
      ...
      var prob = Js_math.random_int(0, 100);
      if (prob < 5) {
...
function generate_ground(blockw, blockh, _inc, _acc) {
  while(true) {
    ...
    if (inc > blockw) {
      ...
      var skip = Js_math.random_int(0, 10);
